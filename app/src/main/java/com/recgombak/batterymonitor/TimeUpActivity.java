...
public class TimeUpActivity extends Activity {
 ...
 private void build(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setGravity(Gravity.CENTER);root.setPadding(dp(24),dp(24),dp(24),dp(24));root.setBackgroundColor(Color.rgb(180,0,0));TextView icon=tv("⛔",72,true);root.addView(icon);TextView title=tv("MASA TAMAT",34,true);root.addView(title);TextView sub=tv("Masa penggunaan tablet telah tamat.",20,false);sub.setPadding(0,dp(12),0,dp(24));root.addView(sub);Button ok=new Button(this);ok.setText("OK");ok.setTextSize(18);ok.setOnClickListener(v->finish());root.addView(ok,new LinearLayout.LayoutParams(-1,dp(56)));setContentView(root);}
